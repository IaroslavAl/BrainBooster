//
//  NumberMemoryEasyViewController.swift
//  BrainBooster
//
//  Created by Iaroslav Beldin on 26.04.2023.
//

import UIKit

final class NumberMemoryEasyViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for textField in numberTextFields {
            ...
            (textField as? CustomTextField)?.numberMemoryEasyVC = self
        }
    }
    ...
    private func finish() {
        ...
        updateRecord(for: .numberMemory, mode: .easy, timeType: countdownTime, score: score)
    ...
    private func updateRecord(for gameType: GameType, mode: Mode, timeType: Int, score: Int) {
        ...
        if let index = games.firstIndex(where: { $0.type == gameType && $0.mode == mode && $0.timeType == timeType }) {
            var gameToUpdate = games[index]
            
            if let gameScore = gameToUpdate.score, score > gameScore {
...
extension NumberMemoryEasyViewController: UITextFieldDelegate {
...
extension NumberMemoryEasyViewController: UIPickerViewDelegate {
...
extension NumberMemoryEasyViewController: UIPickerViewDataSource {
